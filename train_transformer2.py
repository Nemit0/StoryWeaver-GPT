...
# Configure logging
log_filename = f"./logs/{datetime.now().strftime('%Y%m%d%H%M%S')}_transformer1.log"
logging.basicConfig(
...
def main():
    ...
    if os.path.exists('./logs/train_config1.json'):
        with open('./logs/train_config1.json', 'r') as f:
    ...
    if os.path.exists('./model/gpt_model.pth'):
        GptObj = GPT.load_model('./model/gpt_model.pth')
    ...
    # Load data
    with open(os.path.join(os.getcwd(), "./data/combined_data.txt"), "r", encoding="utf-8") as f:
        text = f.read()
    ...
    # Save the model
    GptObj.save_model('./model/gpt_model.pth')
    logging.info("Model saved to file")
    with open('./logs/train_config1.json', 'w') as f:
