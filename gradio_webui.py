...
torch_model_path_finetune = "./model/checkpoints/gpt_model_finetuned.pth"
...
if os.path.exists(torch_model_path_finetune):
    checkpoint_finetune = torch.load(torch_model_path_finetune, map_location=device)
    embedding_dim = checkpoint_finetune.get('embedding_dim', 1024)
    max_seq_len = checkpoint_finetune.get('max_seq_len', 1024)
    heads = checkpoint_finetune.get('heads', 8)
    ff_expand_dim = checkpoint_finetune.get('ff_expand_dim', 2)
    blocks = checkpoint_finetune.get('blocks', 2)
    lr = checkpoint_finetune.get('lr', 0.001)

    model_torch_finetune = GPTTorch(vocab_size=len(tokenizer.token_map),
                embed_size=embedding_dim, 
                max_seq_len=max_seq_len, 
                num_heads=heads, 
                ff_expand=ff_expand_dim, 
                num_blocks=blocks, 
                dropout=0.1).to(device)
    model_torch_finetune.load_state_dict(checkpoint_finetune['model_state_dict'])
else:
    import os
    raise Exception("Model not found")
...
def text_generation(input_text, temperature_value, repetition_penalty_value, model_name):
    ...
    elif model_name == "Pytorch Model Finetuned":
        result = generate_text_torch(
            model_torch_finetune,
            tokenizer,
            input_text,
            max_tokens=512,
            temperature=temperature_value,
...
def main():
    with gr.Blocks() as demo:
        ...
        dropdown = ["Base Model", "Pytorch Model", "Pytorch Model Finetuned"]
